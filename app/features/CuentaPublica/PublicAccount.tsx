import { Database, Plus } from "lucide-react";
import { useEffect, useState } from "react";
import { toast } from "sonner";
import {
  createDatapointPublicAccount,
  getPublicAccounts,
} from "~/api/PublicAccount/publicAccount";
import { Button } from "~/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "~/components/ui/card";
import type {
  DataPointCP,
  FileDataCP,
} from "~/interfaces/PublicAccount/publicAccount";
import { DataSummary } from "./Data-summary";
import { YearManager } from "./Year-manager";

export const PublicAccount = () => {
  const [newYear, setNewYear] = useState<string>("");
  const [dataPoints, setDataPoints] = useState<DataPointCP[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const info = await getPublicAccounts();
        setDataPoints(info);
      } catch (error) {
        console.error("Error cargando cuentas públicas:", error);
      }
    };

    fetchData();
  }, []);

  const addYear = async () => {
    const year = Number.parseInt(newYear);
    if (!year || year < 2000 || year > 2100) {
      toast.error("El año debe estar entre 2000 y 2100");
      return;
    }

    const existingYear = dataPoints.find((d) => d.year === year);
    if (existingYear) {
      toast.error("El año ya existe");
      return;
    }

    await createDatapointPublicAccount(year);

    const periods: DataPointCP["period"][] = ["ANUAL", "Q1", "Q2", "Q3", "Q4"];
    const newDataPointsForYear: DataPointCP[] = periods.map((period) => ({
      year,
      period,
      active: true,
      fileData: undefined,
    }));

    setDataPoints((prev) =>
      [...prev, ...newDataPointsForYear].sort(
        (a, b) =>
          b.year - a.year ||
          periods.indexOf(a.period) - periods.indexOf(b.period)
      )
    );

    setNewYear("");
    toast.success("Año agregado correctamente.");
  };

  const deleteYear = (year: number) => {
    const yearData = dataPoints.filter((d) => d.year === year);
    const filesCount = yearData.filter((d) => d.fileData).length;
    setDataPoints((prev) => prev.filter((d) => d.year !== year));
    toast.info(
      `El año ${year} ha sido eliminado completamente${filesCount > 0 ? ` junto con ${filesCount} archivo(s)` : ""}. Esta acción no se puede deshacer.`
    );
  };

  const updateFile = (
    year: number,
    period: DataPointCP["period"],
    fileData?: FileDataCP
  ) => {
    setDataPoints((prev) =>
      prev.map((d) =>
        d.year === year && d.period === period ? { ...d, fileData } : d
      )
    );

    if (fileData) {
      const sizeInMB = (fileData.size / (1024 * 1024)).toFixed(2);
      toast.info(
        `El archivo ${fileData.name} ha sido actualizado con exito, el tamaño es de ${sizeInMB} MB`
      );
    } else {
      toast.info(
        `Se ha removido el archivo del periodo ${period} del año ${year}. El periodo ahora esta vacio.`
      );
    }
  };

  const getYears = () => {
    const years = [...new Set(dataPoints.map((d) => d.year))].sort(
      (a, b) => b - a
    );
    return years;
  };
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4">
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="text-center space-y-2 py-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            <div className="p-3 bg-blue-600 rounded-xl">
              <Database className="w-8 h-8 text-white" />
            </div>
          </div>
          <h1 className="text-4xl font-bold text-slate-900 text-balance">
            Cuenta Publica
          </h1>
          <p className="text-lg text-slate-600 max-w-2xl mx-auto text-pretty">
            Gestiona los datos históricos organizados por año y período
          </p>
        </div>

        <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
          <CardHeader className="pb-4">
            <CardTitle className="text-xl text-slate-800">
              Agregar Nuevo Año
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="flex flex-col sm:flex-row gap-4 items-end">
              <div className="flex-1 space-y-2">
                <label className="text-sm font-medium text-slate-700">
                  Año a agregar
                </label>
                <div className="flex gap-3">
                  <input
                    type="number"
                    value={newYear}
                    onChange={(e) => setNewYear(e.target.value)}
                    placeholder="2025"
                    className="flex-1 px-4 py-2.5 border border-slate-200 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                    min="1900"
                    max="2100"
                  />
                  <Button
                    onClick={addYear}
                    className="shrink-0 bg-blue-600 hover:bg-blue-700 px-6"
                  >
                    <Plus className="w-4 h-4 mr-2" />
                    Agregar Año
                  </Button>
                </div>
                <p className="text-xs text-slate-500">
                  Se crearán automáticamente los 5 períodos: ANUAL, Q1, Q2, Q3,
                  Q4
                </p>
              </div>
            </div>
          </CardContent>
        </Card>

        <DataSummary dataPoints={dataPoints} />

        <div className="space-y-6">
          {getYears().length === 0 ? (
            <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
              <CardContent className="py-12 text-center">
                <div className="space-y-4">
                  <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto">
                    <Database className="w-8 h-8 text-slate-400" />
                  </div>
                  <div>
                    <h3 className="text-lg font-medium text-slate-900 mb-2">
                      No hay años configurados
                    </h3>
                    <p className="text-slate-600">
                      Agrega un año para comenzar a gestionar los datos
                      históricos
                    </p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ) : (
            getYears().map((year) => (
              <YearManager
                key={year}
                year={year}
                dataPoints={dataPoints.filter((d) => d.year === year)}
                onUpdateFile={updateFile}
                onDeleteYear={deleteYear}
              />
            ))
          )}
        </div>
      </div>
    </div>
  );
};
